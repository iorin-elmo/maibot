...
`/maimai fsync` のブックマークレットは「レコード ＞ 楽曲スコア ＞ version」の全バージョン・全難易度を読み取り、カタログ上の最新2バージョンを新曲、それ以前を旧曲としてBest 15・Best 35を計算します。現在は `CiRCLE PLUS` と `MAGiCAL` が新曲枠です。新しいバージョンへ切り替わった後は、`/maimai fsync` をもう一度実行して保存済みの分類を更新してください。`/maimai sync` はでらっくすRatingページで実行してください。どちらも1回・10分間だけ有効で、同期データは実行したDiscordアカウントに紐付きます。
