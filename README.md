...
`/maimai sync` のブックマークレットは「レコード ＞ 楽曲スコア ＞ version」の全バージョン・全難易度を読み取り、下2バージョンを新曲、それ以前を旧曲としてBest 15・Best 35を計算します。ブックマークレットは1回・10分間だけ有効で、同期データは実行したDiscordアカウントに紐付きます。同期後は、コマンドを実行したチャンネルに初回案内または前回からの差分を通知します。
